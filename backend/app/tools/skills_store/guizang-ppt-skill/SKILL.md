...
description_zh: "生成横向翻页网页PPT（单HTML文件），内含WebGL背景、章节幕封、数据大字报、图片网格等模板。提供两种风格可选：①“电子杂志 × 电子墨水”（衬线字体+流体背景+暖色系）②“瑞士国际主义”（无衬线字体+网格点阵+国际克莱因蓝/柠檬黄/柠檬绿/安全橙高亮）。适用于用户制作分享、演讲或发布会风格的网页PPT，或提及“杂志风PPT”“瑞士风PPT”“Swiss Style”“horizontal swipe deck”时使用。"
description_en: "歸藏's PPT Skills — Generate polished single-file HTML slide decks and cover pages with editorial magazine and Swiss international design systems."
version: 1.1.0
triggers: ppt, deck, 幻灯片, slides, 瑞士, 杂志风, 发布会, 演讲, swiss, magazine, 翻页, 演示文稿, 网页ppt, 横向翻页
category: Design Tools
author: op7418
permissions:
  - file_write
  - optional_network_assets
permission_notes_zh: 本 Skill 会在用户工作区复制模板、创建 images 目录并写入 HTML 文件;模板预览时可能加载公开字体、图标或动效 CDN,如需离线可替换为本地资源。
display_name: "网页 PPT 生成"
...
- 模板：`<SKILL_ROOT>/assets/template.html` · 主题色：`<SKILL_ROOT>/references/themes.md` · 布局：`<SKILL_ROOT>/references/layouts.md`
...
- 模板：`<SKILL_ROOT>/assets/template-swiss.html` · 主题色：`<SKILL_ROOT>/references/themes-swiss.md` · 布局：`<SKILL_ROOT>/references/layouts-swiss.md`
...
- 在对话中直接询问用户即可，不需要依赖任何专用问答机制。
- 一次最多问 1-3 个最关键问题;如果信息缺口不影响开工,先做合理假设并在回复里说明。
- 需要用户提供素材时,直接说明文件应放在工作区 `ppt/images/` 下、如何命名、推荐比例和替换方式。
...
- **文件夹位置**:当前工作区根目录下 `ppt/images/`(和 `ppt/index.html` 同级;`run_command` 的 cwd 默认就是工作区根,无工作区时用后端当前目录)
...
本版本不包含图片生成提示词,也不要求 AI 调用图像模型。图片只作为用户提供或项目已有的本地素材插入 deck。
...
- 先看 `<SKILL_ROOT>/references/image-assets.md` 选择图片类型、比例和落位策略
- 图片比例必须匹配最终落位:主视觉 16:9,左文右图 16:10 / 4:3,信息图 16:9 / 16:10,截图 16:10,图文混排小图 3:2 / 3:4,网格图统一高度裁切
- 图片放到 `images/` 下,命名遵守 `{页号}-{语义}.{ext}`

### Step 2 · 一键构建(复制模板 + 注入 + 自检)

**不要手动复制模板、不要自己拼装 HTML、不要用 write_file 重写整个模板文件**——模板约 100KB,全量重写极易写坏 WebGL shader。所有构建由 `<SKILL_ROOT>/scripts/build.mjs` 一步完成:

```bash
# 风格 B · 瑞士国际主义风(默认推荐)
node "<SKILL_ROOT>/scripts/build.mjs" --style swiss --out "ppt/index.html" --slides "ppt/_slides.html" --title "PPT 标题"

# 风格 A · 电子杂志风(改用 --style a)
node "<SKILL_ROOT>/scripts/build.mjs" --style a --out "ppt/index.html" --slides "ppt/_slides.html" --title "PPT 标题"
```

`build.mjs` 自动完成(任一环节失败会明确报错退出,不会交付带病文件):
1. 复制对应模板到 `--out`(默认 `ppt/index.html`),并创建同级 `images/` 目录
2. 把 slide 片段注入 `<!-- SLIDES_HERE -->`(整段替换模板自带示例页)
3. 用 `--title` 替换 `<title>`
4. **shader 括号自检**(GLSL 括号不平衡立即报错)
5. 风格 swiss/b 时运行 Swiss 版式校验(S01-S22 data-layout)

> 前置条件:先按 Step 3.1 从 layouts 挑好版式,把全部 `<section class="slide ...">` 页面块拼成一个片段,**用 write_file 写入 `ppt/_slides.html`**(只写 section 片段,不要 `<html>/<head>/<style>/<script>`;此文件会出现在产物面板)。然后跑上面一条命令即可。
> Windows 提示:`node` 需在 PATH;若报"命令或解释器不存在",用绝对路径 `"C:\Program Files\nodejs\node.exe"`。
...
模板里的 `<title>` 默认是 "[必填] 替换为 PPT 标题 · Deck Title"——**不需要手动编辑**,build.mjs 的 `--title` 参数会自动替换。构建完成后可用 `cmd /c findstr "必填" "ppt\index.html"` 确认没有遗漏。
...
2. 打开 `<SKILL_ROOT>/references/themes.md`,找到对应主题的 `:root` 块
3. **整体替换** `ppt/index.html`(工作区里已拷贝的模板)开头 `:root{` 块里标有"主题色"注释的那几行(`--ink` / `--ink-rgb` / `--paper` / `--paper-rgb` / `--paper-tint` / `--ink-tint`)
...
1. **先 Read 当前用的模板**(至少读到 `<style>` 块末尾,模板是 `<SKILL_ROOT>/assets/` 下的绝对路径,可用 read_file 的 offset/limit 分段读):
   - 风格 A → `<SKILL_ROOT>/assets/template.html`
   - 风格 B → `<SKILL_ROOT>/assets/template-swiss.html`
...
**在挑布局之前**,必须先列出每一页的主题 class(`hero dark` / `hero light` / `light` / `dark`)并写到文档或草稿里对齐。详细规则看 `<SKILL_ROOT>/references/layouts.md` 开头的"主题节奏规划"一节。
...
**生成后自检**:`cmd /c findstr /C:"class=slide" "ppt\index.html"` 列出所有主题,人工确认节奏合理再交付(cwd=工作区根)。

#### 3.1 · 挑布局

**不要从零写 slide**。打开对应的 layouts 文件,里面有 10 种现成布局骨架,每种都是完整可粘贴的 `<section>` 代码块。

**风格 A** → `<SKILL_ROOT>/references/layouts.md`:
...
**风格 B** → 先读 `<SKILL_ROOT>/references/swiss-layout-lock.md`,再读 `<SKILL_ROOT>/references/layouts-swiss.md`。
...
- 开写 HTML 前先列一张 `页码 → data-layout → 选用理由 → 图片槽位` 草稿;交付前运行 `node "<SKILL_ROOT>/scripts/validate-swiss-deck.mjs" "ppt/index.html"`(cwd=工作区根,node 需在 PATH)。
...
组件细节(字体、颜色、网格、图标、callout、stat-card 等)在 `<SKILL_ROOT>/references/components.md`。

#### 3.3 · 构建与自检（**ChemAgent 流程**)

1. **把全部 `<section class="slide ...">` 页面块拼接成一个片段**,用 **write_file** 写入工作区 `ppt/_slides.html`(只写 section 片段,不要写 `<html>`/`<head>`/`<style>`/`<script>`/WebGL 代码;此文件会作为产物出现在右侧产物面板,方便用户查看)。
2. **run_command 跑一键构建**(推荐,替代手动 inject):

```bash
node "<SKILL_ROOT>/scripts/build.mjs" --style swiss --out "ppt/index.html" --slides "ppt/_slides.html" --title "PPT 标题"
```

   - build.mjs 自动完成:复制模板 → 注入片段 → 替换 title → **shader 括号自检** → Swiss 版式校验,任何一步失败会明确报错,不会交付带病文件。
   - 结构图等复杂页**不要**在片段里写 `<script>` 或自绘 WebGL,一律用模板版式(风格 B 用 S17 system-diagram),动效用模板自带的 `data-anim`/`data-animate` recipe。
3. **构建后检查**:`cmd /c findstr "必填" "ppt\index.html"`(应无输出);若 build 报错,按错误信息修正片段后重跑。
4. 交付时告知用户产物位置:`ppt/index.html`(工作区根),并提示用浏览器打开预览。

> 手动注入备选(需要精细控制时):`node "<SKILL_ROOT>/scripts/inject.mjs" "ppt/index.html" "ppt/index.html" "ppt/_slides.html" --title "标题"`(同样带 shader 自检)。
> 注意:不要用 write_file 直接把 100KB 模板全量重写一遍(内容太大),模板只改占位符,其余结构靠 build 复制+注入。

### Step 4 · 对照检查清单自检

生成完一定要打开 `<SKILL_ROOT>/references/checklist.md`，逐项对照。里面总结了**真实迭代过程中踩过的所有坑**，P0 级别的问题（emoji、图片撑破、标题换行、字体分工）必须全部通过。

#### 4.0 · 不只看代码:必须打开网页做视觉核对

代码只能证明类名和结构存在,不能证明版式舒服。生成后必须打开网页逐页看:

1. 打开生成的 `ppt/index.html`,逐页核对版式(本环境没有原作者的参考 PPT 文件,以模板与 references 描述为准)。
...
直接用浏览器打开 `ppt/index.html`(工作区根)。Windows 下:

```bash
cmd /c start "" "ppt\index.html"
```

不需要本地服务器。图片走相对路径 `images/xxx.png`。也可以在交付回复中把文件路径直接给用户,由用户在工作区/产物面板中打开。
...
├── scripts/
│   ├── build.mjs             ← 【推荐】一键构建:复制模板+注入+shader 自检+Swiss 校验(Step 2)
│   ├── inject.mjs            ← 手动注入:把 slide 片段写进模板占位符(备选)
...
> **路径约定**:上述所有相对路径在指令注入时以 `<SKILL_ROOT>` 为前缀(运行时替换为技能目录的绝对路径),例如模板 = `<SKILL_ROOT>/assets/template.html`。**不要**把这个目录当成工作区使用——产出文件一律写到工作区 `ppt/` 下。

**加载顺序建议**：
1. 先读完 `SKILL.md`(这个文件)了解整体
2. Step 1 需求澄清**第一问**先确定风格 A 还是 B,然后:
   - 风格 A:读 `<SKILL_ROOT>/references/themes.md` 帮用户选一套主题色
   - 风格 B:读 `<SKILL_ROOT>/references/themes-swiss.md` 帮用户选一套主题色
3. **动手前 Read 对应模板的 `<style>` 块**——这是类名的唯一来源,缺类会导致整页样式崩(read_file 支持 offset/limit 分段读)
   - 风格 A → `<SKILL_ROOT>/assets/template.html`
   - 风格 B → `<SKILL_ROOT>/assets/template-swiss.html`
4. 读对应的 layouts 文件挑布局:
   - 风格 A → `<SKILL_ROOT>/references/layouts.md`(顶部有 Pre-flight 类名清单、主题节奏规划、动效 recipe 决策树)
   - 风格 B → **先读 `<SKILL_ROOT>/references/swiss-layout-lock.md`**,再读 `<SKILL_ROOT>/references/layouts-swiss.md`;正文页必须从 S01-S22 选择,每页写 `data-layout`
5. 如果 deck 使用图片素材,读 `<SKILL_ROOT>/references/image-assets.md` 挑图片类型、比例和落位策略
6. 细节调整时读 `<SKILL_ROOT>/references/components.md` 查组件(含 Motion 动效系统章节,主要服务风格 A;风格 B 的组件细节在 `layouts-swiss.md` 附录)
7. 生成流程:write_file 片段到 `ppt/_slides.html` → 跑 `node "<SKILL_ROOT>/scripts/build.mjs" --style swiss --out "ppt/index.html" --slides "ppt/_slides.html" --title "标题"`(Step 2/3.3)→ 读 `<SKILL_ROOT>/references/checklist.md` 自检

**动效相关**:模板已把 Motion One 的加载和 recipe 逻辑内嵌到底部 module script。你不需要改 JS,只需要按 `<SKILL_ROOT>/references/layouts.md` / `<SKILL_ROOT>/references/layouts-swiss.md` 的骨架在 HTML 里加 `data-anim` / `data-animate` 即可。离线演示靠 `<SKILL_ROOT>/assets/motion.min.js`,断网时自动降级为"无动画但内容可读"。风格 B 模板必须保留 `B` 键低功耗模式:切换后停止 WebGL/ASCII canvas RAF,取消正在运行的 Web Animations,并把当前页内容直接 reveal 到静态最终态。
