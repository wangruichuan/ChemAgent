...
本主题的 golden source 是模板种子文件 `<SKILL_ROOT>/assets/template-swiss.html`(本环境无原作者本机参考文件,以模板与 `references/swiss-layout-lock.md` 为准)。
...
- 如果信息图、流程图、UI 情景图已经按 S15/S16 槽位准备成标准比例,必须用 `.frame-img.r-21x9` / `.frame-img.r-16x10` 铺满槽位;不要再加 `.fit-contain`,否则会变成小图漂在白框里
- 瑞士风图片优先比例:S22 顶部横幅 `21:9`;S15/S16 多图格统一 `21:9` 或统一 `16:10`
- 插入 2-3 张配图时,必须先绑定原始版式槽位:单张大图 = S22;多图 = S15/S16 网格改造;不要使用未登记的 P23/P24
- S22 的照片主体必须位于中央安全区,HTML 用 `object-position:center 35%` 或 `center center`,不要用 `top center` 截人脸
- 图片素材必须遵守单一 accent 色、Helvetica/Inter 气质、12/16 列网格、直角纯色、无渐变/阴影/圆角
- 图片只保留核心图像本身,不要把页眉、页脚、标题、页码、角标、边框、署名画进图片里
...
1. 打开生成的 `ppt/index.html`,与 `<SKILL_ROOT>/assets/template-swiss.html` 种子模板对照逐页核对(本环境无原作者本机参考文件,以模板与 references 描述为准)。
2. 截图前先等入场动效稳定(约 1-2 秒)。不要把动画中间态误判成"内容缺失"或"版式空白"。
3. 先看视觉:标题重量、头部距离、图片落位、底部安全区、caption 是否被 nav 挡住。
4. 对照模板种子文件的同类版式,不要只对照 CSS helper;以实际页面结构和视觉结果为准。
...
**注意**:
- 图片优先用 `images/{页号}-{语义}.png` 本地文件,不要默认使用外部图片服务
...
<section class="slide light" data-animate="grid-reveal">
  <div class="canvas-card">
    ...
    <div style="flex:1;padding:0;display:grid;grid-template-rows:auto 1fr;gap:5vh">
      <div data-anim="head" style="display:flex;flex-direction:column;gap:1.4vh">
        <div class="t-meta">Evidence · Visual Asset</div>
...
- 如果用户原始截图比例混乱,优先拆成同尺寸局部面板,或要求用户提供同一比例素材
