...
**现象**:图片像普通 PPT 插图,圆角、阴影、比例混乱;多张截图高度不一,或图片素材自带标题/页脚,和页面 chrome 重复。

**根因**:瑞士风的图片不是装饰,而是 grid 里的证据块。没有先选原始版式和图片槽位,就会把任意图片硬塞进页面。

**做法**:
- 先选版式:单张大图 + KPI 用 `S22`;多图用 `S15/S16` 的原始网格骨架改造
- S22 图片素材比例固定 `21:9`,并在 `<img>` 上写 `data-image-slot="s22-hero-21x9"`
- 照片默认 `object-position:center 35%` 或 `center center`,不要用 `top center` 截人脸
- 图片容器只用 `.frame-img`;**不要** `border-radius` / `box-shadow`
- UI / 信息图 / 流程图若是用户原始截图或文字密集图,使用 `.fit-contain`;若已按槽位准备成标准比例,必须用对应比例类铺满容器,例如 `.frame-img.r-21x9`,不能再用固定短高度把图片缩小
- 多图同组必须统一槽位、比例、高度,不要混用
- 图片素材必须符合 Swiss Style、单一 accent、直角、无渐变/阴影/圆角、无页眉页脚标题角标

**自检命令**:
- `grep -E "frame-img.*border-radius|box-shadow" index.html`——命中就删
- `grep -n "data-image-slot" index.html`——每张本地图片都应有槽位声明
- 目视:图片内部如果自带大标题、页码、页脚、角标,优先更换素材或裁切到核心区域,不要在页面里再叠一层 chrome 硬救
...
**做法**:
- 模板 `assets/template-swiss.html` 是 Swiss 主题的 golden source,以**实际页面用法**为准,不要只看未使用的 CSS helper
...
- 如果确实需要全宽,必须使用比例足够长的横向图片,不要把普通截图强行拉成长条

### 13d. 图片素材不要自带 slide 元素

- 图片只是嵌入素材,不要让图片自带页眉、页脚、标题、页码、角标、署名或装饰边框
- 流程图/信息图只保留核心图形和必要短标签,PPT 自己负责标题、页脚和 chrome
- 如果图片已经带了这些元素,优先换素材或裁切到核心区域;不要在 PPT 里再叠一层 chrome 造成干扰
