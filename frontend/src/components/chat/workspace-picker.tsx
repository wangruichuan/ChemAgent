  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="truncate font-medium">{isNone ? t("ws.none") : (currentWs?.name ?? t("sidebar.workspace"))}</span>
        ...
        <div className="absolute left-0 top-full z-50 mt-1 w-72 rounded-xl border bg-card p-2 shadow-[var(--shadow-md)]">
          ...
          <div className="max-h-52 space-y-0.5 overflow-y-auto">
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block truncate text-xs font-medium">{t("ws.none")}</span>
                <span className="block truncate text-[10px] text-muted-foreground/60">{t("ws.noneDesc")}</span>
              </span>
          ...
          <div className="mt-2 space-y-1.5 border-t border-border/60 pt-2">
            <p className="flex items-center gap-1 px-0.5 text-[10px] font-medium text-muted-foreground/70">
              <FolderPlusIcon className="size-3" /> {t("ws.add")}
            </p>
            <Input
              value={name}
