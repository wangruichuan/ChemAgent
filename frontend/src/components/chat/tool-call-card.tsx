...
/** 解析结果 JSON，返回展示摘要 */
function summarize(result: string): { title: string; failed: boolean } {
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      ...
      return { title: `返回 ${parsed.length} 条结果`, failed: false }
    }
    ...
    if (parsed && typeof parsed === "object" && "exit_code" in parsed) {
      return {
        title: `退出码 ${parsed.exit_code} · ${String(parsed.stdout ?? "").slice(0, 60) || "无输出"}`,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {running && <span className="text-[11px] text-muted-foreground/50">执行中…</span>}
        {!running && (
          <ChevronRightIcon
            className={cn(
